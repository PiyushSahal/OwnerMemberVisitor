...
$csvFilePath = "E:\Scripts\SPO\VisitorList.csv"
...
foreach ($userRow in $userList) {
    ...
    $group = Get-SPOSiteGroup -Site $siteUrl | Where-Object { $_.Roles -contains "Read" } | Select-Object -ExpandProperty Title
    ...
    foreach ($userEmail in $userEmails) {
        $userEmail = $userEmail.Trim()
            ...
            Write-Host "User with email $userEmail added to the default 'Members' group in site $siteUrl"
